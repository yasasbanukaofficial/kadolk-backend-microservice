...
The **single source of configuration** for the whole system. These YAML files are served by the config server (`infastructure/config-server`, `:8888`) to every service — no service keeps its settings in its own repo copy anymore; changing a YAML here (and pushing to the remote repo's `main` branch) reconfigures the running system.

## How it is served

```mermaid
flowchart LR
    CR[config-repo/] -->|cloned by| CS[Config Server :8888]
...
- Java services import it via `optional:configserver:http://localhost:8888` in their local `application.yaml`.
...
| `payment-service.yaml` | port `8084`, datasource `${DB_URL}`, `user-service.url`, `eureka.client.enabled: true` |
...
- **Adding a new service?** Create `{service-name}.yaml` here, and give the service a local `application.yaml` that imports `optional:configserver:http://localhost:8888`.
